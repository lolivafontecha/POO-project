#pragma once
#include <vector> // pour attribut "coord"
#include <iostream> // pour operateur << (iostream)
#include <initializer_list> // pour constructeur
...
class Vecteur {
    ...
    bool compare(Vecteur const& vect) const; /*/ Compare deux vecteurs composante par composante
                                              sous la consdition qu'ils soient de même taille /*/
    ...
    void set_coord(size_t place, double valeur); /*/ modifie la valeur de la place precisée
                                                 (méthode demandée dans l'exercice P1 mais non utilisé
                                                    dans le programme) /*/
    ...
    const Vecteur operator-(Vecteur const& vect) const;
    const Vecteur operator^(Vecteur const& vect) const; // comme demandée, operateur associé au produit vectoriel
    ...
    const Vecteur operator*(double a, Vecteur const& vect); /*/ encore multiplication par un scalaire,
                                                            cette fois ci pour pouvoir multiplier par la droite /*/
